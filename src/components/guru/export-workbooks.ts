...
export function downloadDailyObservationWorkbook(
  ...
  writeWorkbook(
    ...
    rows.map((row) => [
      ...
      row.nisn,
      row.studentName,
      row.respons ? frequencyLabels[row.respons] : '',
      row.interaksi ? frequencyLabels[row.interaksi] : '',
      row.partisipasi ? frequencyLabels[row.partisipasi] : '',
      row.regulasi ? frequencyLabels[row.regulasi] : '',
      row.note,
    ]),
  )
}
