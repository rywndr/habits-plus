...
import { classes, students, users, getTenantBySlug } from '#/data'
...
function BerandaAdmin() {
  ...
  const parentsCount = users.filter((u) => u.role === 'ortu').length
    ...
    <ContentPanel>
      <div className="flex flex-col gap-6">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm text-muted-foreground">{tenantInfo.name}</p>
        </div>

        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <StatCard label="Total Guru" value={teachersCount} icon={GraduationCap} />
          <StatCard label="Total Siswa" value={students.length} icon={Users} />
          <StatCard label="Total Kelas" value={classes.length} icon={BookOpen} />
          <StatCard label="Total Orang Tua" value={parentsCount} icon={UserCircle2} />
        </div>
